...
def zeige_process_mining_seite(
    ...
    try:
        ...
        if zustand["schritt"] == 2:
            ...
            k = float(
                st.slider(
                    ...
                    min_value=0.0,
                    max_value=1.0,
                    value=float(zustand.get("schwellwert_k", 0.0)),
                    step=0.01,
                )
            )
            notation = st.radio(
                ...
                index=list(Prozessnotation).index(
                    zustand.get("prozessnotation", Prozessnotation.PROZESSBAUM)
                ),
            ...
            if zustand.get("vorschau_signatur") != signatur:
                zustand["vorschau"] = process_mining_service.vorschau(freigabe_id, konfiguration)
        ...
        if analyse is None:
            ...
            _dfg(vorschau)
            _prozessmodell(vorschau)
            if st.button("P und A_D reproduzierbar speichern", type="primary"):
